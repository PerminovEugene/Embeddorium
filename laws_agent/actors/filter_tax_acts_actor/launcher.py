...
from laws_agent.actors.filter_tax_acts_actor.handler import (
...
def filter_tax_acts(*, crawl_target_id: str, group: str) -> None:
    target = sql_store.crawl_targets.get(uuid.UUID(crawl_target_id))
    log_dir = target.log_dir if target is not None else None

    with log_to(log_dir):
        _filter_tax_acts(crawl_target_id=crawl_target_id, group=group, store=sql_store)
