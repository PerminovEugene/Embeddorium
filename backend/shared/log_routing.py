...
@contextmanager
def log_to(
    log_dir: Optional[str], pipeline_id: Optional[str] = None
) -> Iterator[None]:
    """Activate per-URL file routing for the current context.

    ``log_dir`` selects the nested per-URL folder for this target; it is a
    no-op component when ``None`` (e.g. the target has none set yet).

    ``pipeline_id`` scopes the run root to
    ``<PIPELINE_RUNS_DIR>/<pipeline_id>/logs/``; when absent the global
    ``LOG_ROOT`` is used as the base.

    Either or both arguments may be ``None`` — callers can always wrap their
    handler body in this context manager unconditionally.
    """
    log_dir_token: Optional[Token] = None
    pipeline_id_token: Optional[Token] = None

    if log_dir is not None:
        log_dir_token = _current_log_dir.set(log_dir)
    if pipeline_id is not None:
        pipeline_id_token = _current_pipeline_id.set(pipeline_id)

    try:
        yield
    finally:
        if log_dir_token is not None:
            _current_log_dir.reset(log_dir_token)
...
class ContextRoutingFileHandler(logging.Handler):
    ...
    (where ``log_root`` is the value passed to the constructor, defaulting
    to the module-level ``LOG_ROOT``).

    Both ``log_dir`` and ``pipeline_id`` are read from
    ``contextvars.ContextVar`` on every ``emit``, so concurrent dramatiq
    worker threads each route to their own file without interfering with one
    another. When no ``log_dir`` context is active the handler does nothing —
    stdout already covers that case.
    """
    ...
    def emit(self, record: logging.LogRecord) -> None:
        ...
        try:
            pipeline_id = _current_pipeline_id.get()
            file_path = self._file_path_for(log_dir, pipeline_id)
    ...
    def _file_path_for(self, log_dir: str, pipeline_id: Optional[str]) -> Path:
        ...
        return base / log_dir / f"{leaf}.log"
