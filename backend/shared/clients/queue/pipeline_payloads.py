...
Every stage after ``fetch_source`` is driven by the crawl target id alone; the
stage looks up whatever else it needs (source fetch, document, chunks) from
the store. Keeping the wire payload minimal avoids carrying large or stale
data through RabbitMQ.

``pipeline_id`` is threaded through every payload so each actor can load the
run's saved configuration (dataset, provider/model, chunk settings, collection
name) by id instead of falling back to global env config.
"""
...
class TrackPipelineStatusPayload:
    ...
    Unlike the crawl-stage payloads above this carries no target id — the
