...
Uses FastAPI's TestClient against the real router (no live DB): ``SqlStore``
is patched at the import site in ``search.history`` so every handler receives
a MagicMock store.
"""
...
def test_list_searches_joins_run_and_input_info():
    ...
    with patch(
        "backend.server.search.history.SqlStore",
        return_value=_mock_store(_make_search()),
    ):
...
def test_get_search_returns_stored_results_in_order():
    ...
    with patch(
        "backend.server.search.history.SqlStore",
        return_value=_mock_store(_make_search(results=hits)),
    ):
...
def test_get_unknown_search_returns_404():
    ...
    with patch(
        "backend.server.search.history.SqlStore", return_value=_mock_store(None)
    ):
