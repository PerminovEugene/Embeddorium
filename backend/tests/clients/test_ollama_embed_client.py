...
def _make_client(**kwargs) -> OllamaEmbedClient:
    with patch("backend.shared.clients.ollama_embed_client.OllamaEmbeddings"):
...
def test_constructs_ollama_embeddings_with_model_and_base_url() -> None:
    with patch("backend.shared.clients.ollama_embed_client.OllamaEmbeddings") as mock_cls:
